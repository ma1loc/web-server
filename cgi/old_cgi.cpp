...
Cgi::Cgi()
{
    ...
    safeExit    = true;
    writeEnd    = false;
    // ADD  to close the pipeIn[1] in case you have no body to read from
    // needsInput = false;
    closedAll   = false;
    sigTermSent = false;
    envp        = NULL;
...
void Cgi::checkForCgi(Client &client)
{
    ...
    // TODO: about the path call chage that, to the new one
    scriptPath = client.res.get_path();  // old one
    // {    new one
...
{
    // here you need to init needsInput to now in the parent process
    // if need to close the PipeIn[1] or not
...
void Cgi::parentProcess()
{
    ...
    /*  TODO:
        need to close the pipeIn[1] in case you have no body to read from
        in the case of stuping the CGI most init needsInput(true/false)
        to close it in the parentProcess()
    */
    // if (!needsInput)
    // {
    //     close(pipeIn[1]);
