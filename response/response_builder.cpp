...
std::string response_builder::index_file_iterator(const std::string &full_path)
{
    ...
    std::deque<std::string>::const_iterator it = current_client->location_conf->index.begin();
...
void    response_builder::serving_static_file()
{
    ...
    int file_fd = open(this->path.c_str(), O_RDONLY);
    if (file_fd == -1)
    {
        this->current_client->res.set_stat_code(FORBIDDEN_ACCESS);
        this->set_header();
    ...
    // >> save static_file fd;
    this->current_client->res.set_static_file_fd(file_fd);
    // >> flag that will used later
    this->current_client->is_serving_file = true;
    // >> extract file-size and hold it in 'res'
...
void response_builder::build_response()
{
    ...
    /*
        >> redirection the result into 'this->final_raw_response'
                to SERV it durring EPOLLOUT event
    */
