...
void    socket_engine::client_event(ssize_t fd, uint32_t events) // DONE []
{
    ...
    {
        // 1. Log the event (optional but good for debugging)
        // 2. Remove the client from your map/data structures
        // 3. CLOSE the FD (This automatically removes it from epoll)
        // terminate_client(fd, "Client disconnected or Error occurred"); 
        return ;
    }
